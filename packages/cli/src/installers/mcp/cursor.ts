// ---------------------------------------------------------------------------
// Cursor MCP-client adapter — JSON-write only (Cursor has no public MCP CLI).
//
// Per Cursor's MCP docs (verified at WP-017 implementation time):
//   project scope → `<workspaceDir>/.cursor/mcp.json`
//   user scope    → `~/.cursor/mcp.json`
// Both files share the same `mcpServers.<name>` shape used by the Claude
// project file, so we reuse the chemag-tagged entry from `_json-merge.ts`.
// ---------------------------------------------------------------------------
...
export interface CursorAdapterInjections {
  homedir?: () => string;
}
...
export function getCursorConfigPath(
  ...
  if (scope === "project") return path.join(workspaceDir, ".cursor", "mcp.json");
  return path.join(homedir(), ".cursor", "mcp.json");
}

export function createCursorAdapter(inj: CursorAdapterInjections = {}): ClientAdapter {
  const homedir = inj.homedir ?? os.homedir;
  const configPath = (scope: Scope, workspaceDir: string): string =>
    getCursorConfigPath(scope, workspaceDir, homedir);
  ...
  function install(opts: ClientInstallOpts): ClientInstallResult {
    ...
    return {
      ...
      changed,
      path: "json",
      notes: opts.noCli ? ["--no-cli passed (cursor uses JSON-write only)"] : [],
    };
  }
  ...
  return { id: "cursor" as ClientId, install, uninstall, status };
}
...
export const cursorAdapter: ClientAdapter = createCursorAdapter();
